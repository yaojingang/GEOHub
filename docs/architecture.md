...
`geo-discover`, `geo-content`, `geo-measure`, and `seo` are offline. `geo-diagnose` supports replay snapshots and a tightly bounded explicit-source network mode. `geo-strategy`, `geo-knowledge`, and `geo-publish` have null entrypoints and cannot execute.

The `seo` provider is a dedicated one-line entry for bounded SEO scoping and action planning. It emits no site finding without evidence and performs no live collection or mutation. Future SEO executors can extend its plan stages while preserving the same Registry, evidence, permission, Artifact Bus, output, and degradation contracts.
