...
Scope: GEOHub 0.3.1 local CLI and six active Library-engineered GEO/SEO skill packages.

- Permissions: router read-only; active executors read explicit inputs and write the user-selected runs root.
- Network: discovery, content, measurement, and one-line SEO planning are offline. Diagnosis permits only explicit public canonical HTTP(S) sources behind SSRF, redirect, content-type, size, timeout, and file-descriptor gates.
...
- Research trust: runtime principles resolve to the fixed 54-paper and two-dataset audit; causal labels, proxy variables, platform scope, controls, and limitations stay attached to outputs.
- Measurement trust: supplied observation records retain source URIs, complete missingness and denominator accounting, platform strata, and descriptive interpretation boundaries.
- SEO trust: the deterministic planner produces no site findings, keeps unsupported metrics unknown, and requires matching implementation intent, explicit authorization, and a rollback boundary before marking write scope authorized.
